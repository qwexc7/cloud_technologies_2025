...
В хорошем докерфайле изменили на ```FROM python:3.9-slim``` - фиксированная версия , уже содержит Python , поэтому не нужно устанавливать вручную. Это улучшает стабильность сборки 
...
- ```python3``` - устанавливает интерпретатор Python , в хорошем докерфайле это сделанно с помощью ```FROM python:3.9-slim```
...
- Все данные теряются при пересоздании или удалении контейнера

2. Игнорирование логов
- Невозможно диагностировать ошибки
